...
import '../voice/data/datasources/speech_recognizer.dart';
import '../voice/data/datasources/text_to_speech_datasource.dart';
import '../voice/data/datasources/tts_playback_queue.dart';
import '../voice/data/datasources/voice_setup_channel.dart';
...
class VoiceLoopManager extends Cubit<VoiceLoopState> {
  ...
  Future<void> cancelGeneration() async {
    ...
    _isGenerating = false;
    _checkmarkTimer?.cancel();
    await _aiRepository.cancelGeneration();
    await _ttsQueue.clear();
    await _sttSource.stopListening();
    _avatarController.setIdle();
    emit(state.copyWith(phase: VoiceLoopPhase.idle, showCheckmark: false));
